...
int main() {
    ...
    Renderer2D renderer(rb,camera,proj, Point3D(0,0,-1));

    renderer.render(scene,"lambert");
    rb.save_ppm("cube_lambert.ppm");

    rb.clear_depth(); rb.data.assign(rb.data.size(),0);
    renderer.render(scene,"phong");
    rb.save_ppm("cube_phong.ppm");

    rb.clear_depth(); rb.data.assign(rb.data.size(),0);
    renderer.render(scene,"gouraud");
    rb.save_ppm("cube_gouraud.ppm");

    std::cout << "Rendered Lambert, Phong, Gouraud cubes.\n";
}
